...
- **剧本生成到成片** — 在 Web 工作台中生成、修改、版本化和审批剧本，并从已审批分集直接创建视频剪辑任务。
- **可靠性测试框架** — 支持故障注入、脱敏回放、能力预检以及时长、配音、硬约束和素材下载配额验收。
...
- **`script/story/`** — 剧本素材导入、生成、相似度检查、版本审批和多格式导出。
- **`script/harness/`** — 可靠性场景、故障注入、回放、能力预检和验收规则。

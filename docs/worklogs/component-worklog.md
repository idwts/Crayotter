...
| 2026-08-11 | Claude | 设计语言完全一致化：styles.css 全量色彩审计发现 250+ 个硬编码 hex（品牌紫有 14 个近亲、浅紫底 6 个、灰阶 60+ 个一次性值）→ 五轮归并收敛为 32 色全语义色板；`:root` 新增 `--app-*` token 层（墨色 5 阶/表面 5 阶/品牌 5 件/语义 3 组/类别色/深色面）；tailwind `app.brand` #155EEF→#5E6FEF、`app.success` #0E9384→#22C55E 与 CSS 侧对齐（修复认证页蓝色 vs 工作台靛蓝两套主色）；圆角刻度归一（7/9→8、5→6、14→16）；两处"进行中"渐变方向统一。归并规则：文本色→brand-ink、背景/边框→brand、灰阶按明度映射墨色阶，类别色（sky/violet metric 卡）保留为独立 token。线上全页面截图扫描（工作台/弹层/三列表页/设置/登录）无 JS 错误、无视觉回归；素材高级 E2E + 免责声明 E2E 全过。frontend-style.md §2 重写为唯一色板事实源并登记圆角刻度。 |
| 2026-08-11 | Claude | 文档第二轮全量核对修正：frontend-messages §3.2 请求体改为"前端实发 6 字段+后端可选字段"分离、§4.5 事件日志下载更正为 logDownload.js、§7 更正 /api/auth/password 前端未接、§0 编号重排；frontend-style §3 残留 #155eef 修正；directory-tree 清除 133 个 NUL 损坏字节并补登 demo-showcase.json/latest_skills.md/_vendor/verl。backend-apis 逐条核对匹配。 |
| 2026-08-11 | Claude | 认证安全 + 账号闭环（高优先级迭代）：①防爆破——auth.py 新增线程安全 `FailureLockout`（登录/重置同一 IP+账号 10 分钟失败 5 次锁定 15 分钟，注册每 IP 每小时 10 次），do_POST 新增 `AuthThrottledError→429 {error, retry_after}` 映射（429 首次进入错误码表）；②设置弹窗新增「账号安全」页签（ChangePasswordSection：原密码+新密码+确认，调 POST /api/auth/password，成功后端吊销全会话、前端回登录页）；③修复静默失败 bug——认证页（登录/注册/重置）未渲染 ToastViewport，失败时 notify 无处显示，用户完全无反馈。验证：锁定器单测 3 项、test_auth_security.py 14 步（429+retry_after/锁定期正确密码也拒/改密全链路）、test_e2e_change_password.py 5 步截图、认证 E2E 13 步线上回归、免责声明 E2E 全过。前后端已部署重启。 |
...
| 2026-08-19 | Claude | Should-Fix 收尾（设计评审遗留）：①任务历史空状态副文案由重复页头副标题改为行动引导（noJobsHint）；②素材库上传两按钮窄屏（≤640px）仅图标显示，避免挤压标题区（.materials-upload-label）；③accesslint/axe 引擎扫描登录/注册/用户协议页 0 违规（本地静态构建注入扫描，工作台页因服务器失联待补扫）。注：服务器 8.161.229.68 于 18:10 起全端口失联（疑似本地 clash-verge TUN 故障或实例问题），本次改动仅本地验证构建通过，待服务器恢复/更换后部署。 |

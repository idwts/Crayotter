...
| requirements.txt | aiohappyeyeballs==2.6.1 |
...
| tests/test_auth_security.py | 认证防爆破+改密链路 API 测试（2026-08-11） |
...
| website/assets/demo-showcase.json | 演示案例数据（website/assets 下 log_calls/logs/traces/videos 为运行时/大型产物目录，按排除条款不展开） |
| memory_experience/latest_skills.md | 经验沉淀快照（非运行时） |
| _vendor/verl/ | 第三方 verl 源码参考（vendored，非本项目代码） |